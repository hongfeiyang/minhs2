...
showSummary marks =
  color brightWhite $
    if length marks > 0
      then
        "Passed "
          ++ show (length $ filter (/= 0) marks)
          ++ " out of "
          ++ show (length marks)
          ++ " tests: "
          ++ show (((length $ filter (/= 0) marks) * 100) `div` length marks)
          ++ "% Correct. Total of "
          ++ show (sum marks)
          ++ " marks."
      else "No tests run."
  ...
  marks <-
    foreach tests $
      ( \test -> do
          ...
          (exit, out, err) <- readCreateProcessWithExitCode (shell (unwords $ exe : (flags ++ [test]))) ""
          let check = do
                r1 <- doCheck ".out" "Stdout" test out
            ...
            ExitFailure i ->
              if expect_fail
                then check
                  ...
                  putStrLn $ color darkRed ("Executable returned non-zero exit code(" ++ show i ++ ").")
                  dumpOutput err out
            ExitSuccess ->
              if not expect_fail
                then check
                else do
                  putStrLn $ color darkRed ("Expected program failure, but it unexpectedly succeeded.")
                  dumpOutput err out
      )
...
getMarks filename =
  let readInteger s = case reads s of
        [(a, b)] -> a
        _ -> 1
   in do
        v <- doesFileExist filename
        if v
          then readInteger <$> readFile filename
          else return 1
...
main = do
  ...
  when (v == ["--help"]) $ do
    putStrLn $
      "Liam's Haskelly Test Runner v0.1. \n"
